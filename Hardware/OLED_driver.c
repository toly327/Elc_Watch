...
{	
	static uint8_t Brigh=1;

	if(Brightness_UP)
	{	
		Brigh++;
		OLED_Brightness(Brigh);
		if(Brigh>=OLED_UI_Brightness){Brigh=1;Brightness_UP=false;}
	}
}
...
void OLED_Power(uint8_t Data)
{
	if(Data)
	{
		...
		OLED_Write_CMD(0x8B);    //0x8B开启充电泵	
		
		OLED_Write_CMD(0xAF);	//0xAF开启显示
	}
	else
	{	
