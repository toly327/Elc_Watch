 ...
 */
/*OLED_UI全局变量定义 */
bool OLED_UI_EN = false;                                                //用于控制oledui是否显示
...
int16_t OLED_UI_Brightness = 255;									//全局变量，存储当前屏幕亮度
...
uint8_t d1=15,d2=1,yiup=0,xiup,ytup=0,xtup,s1;
 //按键识别，返回1为下，-1为上  //如果摇杆下拨                                                                                                                                                      //如果摇杆上拨                        																							//您搁这开火车呢？？？(指作者自己)																				
int8_t menu_Roll_event(void){if(GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)==1){if(yiup){if(xiup>s1){s1=d2;xiup=0;return 1;}else{xiup++;return 0;}}else{yiup=1;xiup=0;s1=d1;return 1;}}else{yiup=0;}if(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_13)==1){if(ytup){if(xtup>s1){s1=d2;xtup=0;return -1;}else{xtup++;return 0;}}else{ytup=1;xtup=0;s1=d1;return -1;}}else{ytup=0;}return 0;}																																					//不展开完全没有可读性 o(><；)oo																		//好像展开也读不了多少(+_+)?
uint8_t yu=0,xu,upp=0,s2;
//按键识别，返回1确定，2长按
int8_t menu_Enter_event(void){if(GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_1)==1){if(upp==0){if(yu){if(xu>s2){upp=1;return 2;}else{xu++;return 0;}}else{yu=1;xu=0;s2=d1;return 0;}}else{return 0;}}else{if(yu){if(upp){yu=0;upp=0;return 0;}else{yu=0;return 1;}}return 0;}}
...
void OLED_DrawRectangle2(uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height)
{
	OLED_DrawLine(X,Y,X,Y+3);
	OLED_DrawLine(X,Y,X+3,Y);
	
	OLED_DrawLine(X+Width-3,Y,X+Width,Y);
	OLED_DrawLine(X+Width,Y,X+Width,Y+3);
	
	OLED_DrawLine(X,Y+Height-1,X,Y+Height-1-3);
	OLED_DrawLine(X,Y+Height-1,X+3,Y+Height-1);
	
	OLED_DrawLine(X+Width-3,Y+Height-1,X+Width,Y+Height-1);
	OLED_DrawLine(X+Width,Y+Height-1,X+Width,Y+Height-1-3);
}
...
void PrintMenuElements(void){
	...
	if(page->General_MenuType == MENU_TYPE_LIST){
		...
		OLED_DrawLine(TempTargetArea.CurrentArea.X + TempTargetArea.CurrentArea.Width-2- 3,TempTargetArea.CurrentArea.Y,TempTargetArea.CurrentArea.X + TempTargetArea.CurrentArea.Width- 3+2,TempTargetArea.CurrentArea.Y);
		OLED_DrawLine(TempTargetArea.CurrentArea.X + TempTargetArea.CurrentArea.Width-2- 3,TempTargetArea.CurrentArea.Height + TempTargetArea.CurrentArea.Y-1,TempTargetArea.CurrentArea.X + TempTargetArea.CurrentArea.Width- 3+2,TempTargetArea.CurrentArea.Height + TempTargetArea.CurrentArea.Y-1);
	}
	...
	if(page->General_MenuType == MENU_TYPE_TILES){
		...
		for(MenuID i = 0; i<num;i++)
		{
			if(CursorPoint.X + page->Tiles_TileWidth < 0 || CursorPoint.X > OLED_WIDTH)
			{
				...
				ceil(CursorPoint.X)/2+60,
				CursorPoint.Y+ceil(CursorPoint.X)-40,
			
				page->Tiles_TileWidth,
				page->Tiles_TileHeight,
			
				0,
				0,
				page->Tiles_ScreenWidth,
				page->Tiles_ScreenHeight,
		...
		static MenuID JIY ,Direction;
		static float Indicator_strip=0 ,upX=0 ,upY=0;
		
		if(page->_ActiveMenuID > JIY)
		{Direction=1;Indicator_strip=20;}
		else if(page->_ActiveMenuID < JIY)
		{Direction=-1;Indicator_strip=30;}
				
		JIY=page->_ActiveMenuID;
		
		upX = 0.04 * Indicator_strip + (1 - 0.04) * upX;
		upY = 0.1 * Indicator_strip + (1 - 0.1) * upY;
		
		if(upY>15)Indicator_strip=0;
	
		if(Indicator_strip>upX){if(Indicator_strip-upX<0.5)upX=Indicator_strip;}
		else{if(upX-Indicator_strip<0.5)upX=Indicator_strip;}
		
		if(Indicator_strip>upY){if(Indicator_strip-upY<0.5)upY=Indicator_strip;}
		else{if(upY-Indicator_strip<0.5)upY=Indicator_strip;}
						
		OLED_DrawRectangle2(upX*Direction+80,upY*Direction+11,38,38);
		...
		UPKK = 0.1 * StringLength + (1 - 0.1) * UPKK;
		
		OLED_DrawLine(-UPKK/1.5+(StringLength > page->Tiles_ScreenWidth? 0 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip : page->Tiles_ScreenWidth/2 - CalcStringWidth(ChineseFont,ASCIIFont,page->General_MenuItems[page->_ActiveMenuID].General_item_text)/2 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip),63,-upY/1.5+(StringLength > page->Tiles_ScreenWidth? 0 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip : page->Tiles_ScreenWidth/2 - CalcStringWidth(ChineseFont,ASCIIFont,page->General_MenuItems[page->_ActiveMenuID].General_item_text)/2 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip)+UPKK-20,63);
		OLED_DrawLine(-UPKK/1.5+(StringLength > page->Tiles_ScreenWidth? 0 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip : page->Tiles_ScreenWidth/2 - CalcStringWidth(ChineseFont,ASCIIFont,page->General_MenuItems[page->_ActiveMenuID].General_item_text)/2 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip),62,-upY/1.5+(StringLength > page->Tiles_ScreenWidth? 0 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip : page->Tiles_ScreenWidth/2 - CalcStringWidth(ChineseFont,ASCIIFont,page->General_MenuItems[page->_ActiveMenuID].General_item_text)/2 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip)+UPKK-20,62);
		
		OLED_PrintfMixArea(0,0,page->Tiles_ScreenWidth,page->Tiles_ScreenHeight,
		
		upY-6+(StringLength > page->Tiles_ScreenWidth? 0 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip : page->Tiles_ScreenWidth/2 - CalcStringWidth(ChineseFont,ASCIIFont,page->General_MenuItems[page->_ActiveMenuID].General_item_text)/2 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip)-20,
		page->Tiles_ScreenHeight - page->General_FontSize - TILES_BOTTOM_DISTANCE-4 + upY*1.5,
		...
		Time_page_small_Timer(2,1);
		Time_page_BAT(0,11);
		
		OLED_DrawLine(42+18,0,42+18,10);
		OLED_DrawLine(42+18,10,35,22);
		OLED_DrawLine(35,22,0,22);
...
MenuID_Type OLED_KeyAndEncoderRecord(void){
	...
	static bool IfEnterTapAndHold = false;
	//记录上一轮按键状态
	...
	static int16_t EnterPressCounter = 0;
	...
	//如果上一次和这一次都按下的状态持续
		...
		//如果上一次和这一次都按下的状态持续
		 if(OLED_UI_LastKey.Enter == 0 && OLED_UI_Key.Enter == 0 && IfEnterTapAndHold == false){

	        if(EnterPressCounter >= (float)PRESS_TIME * 50){
	            
				
				OLED_UI_LastKey.Back=0;
				OLED_UI_Key.Back=1;		
				
	            IfEnterTapAndHold = true;
	            EnterPressCounter = 0;

	        }else EnterPressCounter++;


	    }else{

			if(OLED_UI_Key.Enter == 1 && IfEnterTapAndHold)
			{	
				OLED_UI_LastKey.Enter=1;
				IfEnterTapAndHold = false;
			}
	...
	if((OLED_UI_Key.Up != OLED_UI_LastKey.Up && OLED_UI_Key.Up == 0) || IfUpTapAndHold){
		ActiveMenuID--;
	}

	//如果检测到【下】按键的上一状态与这次的状态不同，且这一状态是抬起状态，说明用户按下了【下】按键，并且刚刚才抬起
	if((OLED_UI_Key.Down != OLED_UI_LastKey.Down && OLED_UI_Key.Down == 0) || IfDownTapAndHold){
		ActiveMenuID++;
	}
...
extern bool timedllKK;
extern uint8_t timedll[];
extern bool VIDEOOKK;
 ...
 * @return 无
 */
void OLED_UI_InterruptHandler(void){//20ms
	// 获取当前屏幕刷新率
    GetFPS();
	
	static uint8_t DEEL1=100,DEEL2=100,DEEL3=100,VID=0;
...
//			AHT20_Get_data();//获取温湿度信息
			...
			DEEL2=0;
			if(DEEL3<5)DEEL3++; //200*5*5 ms  //25s
			else
			{			
	//			MyRTC_ReadTime();
