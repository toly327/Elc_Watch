...
{	
	...
	//计算电压
		...
		if(i>25)
		{
			...
			OLED_ShowFloatNum(33, 16, voltage, 2, 3, OLED_8X16_HALF);
			OLED_ShowString(89, 24, "V", OLED_6X8);
		
			OLED_ShowFloatNum(0, 56, RTI, 1, 3, OLED_6X8);
		
		    OLED_ShowString(50, 56, "Critical:   %", OLED_6X8);
	   ...
	   if( p == 1){ RTI=RTI+0.001; }	 
		...
		{  
			...
			temp &= 0x00FF; // 保留后 8 位，清除前 8 位 
			temp |= ((uint16_t)0xAD << 8); //左移并合并 //BKP_DR5的标志位为RTI与VIN同用
			BKP_WriteBackupRegister(BKP_DR5, temp);//在备份寄存器写入自己规定的标志位，用于判断是不是第一次执行配置
...
uint8_t key=1,onkey=0;//功能选择判断标志位
uint8_t Sampling = 1;//是否开启采样标志位1开启，0停止
uint8_t Samplingmode=1;//采样模式，1为continuous Mode连续采样，0为Single Mode单次采样
float RangeUP = 5 ,RangeDOWH = 0;//电压量程范围
float Threshold_voltage = 2.1;//触发阈值电压
uint8_t Trigger_Slope=1;  //触发条件，1为上升沿，0为下降沿
uint8_t Fre = 4 , upFre;   //采样时间选择标志位：1=每格10us，2=每格20us，3=每格50us，4=每格100us，5=每格200us，6=每格500us，7=每格1ms，8=每格2ms，9=每格5ms，10=每格10ms，11=每格20ms，12=每格50ms，13=每格100ms，14=每格200ms，15=每格500ms	
float maxvoltage=0,littlevoltage=25;//已检测到的最大最小电压
uint8_t WaP = 0;   //波形在屏幕上的位置0—128
uint8_t edge = 1; //开始寻找上升沿或下降沿标志位

uint16_t WaveForm[250];//存放波形数据的数组(原始AD值)//实际上只用了前200个位置，后面的位置是防止可能采样太快刹不住车？
...
{	
	...
	for(uint8_t i=0;i<200;i++) { WaveForm[i] = 0;}
	
	Voltage_Waveform_Init();
	
	TIM_Cmd(TIM4, DISABLE);//先把tim4关了
...
//        WaveForm[i/6] = (uint16_t)(lower_limit + normalized_value * (upper_limit - lower_limit)); // 缩放到 [lower_limit, upper_limit]  
//    }  
//	
	...
	while(1)
	{
		...
		OLED_ShowFloatNum(-6,0,RangeUP,2,1,OLED_6X8);//这里可能会提示警告，可以不管它	
		
		if(Sampling)OLED_ShowString(0,9,"-ON-",OLED_6X8);
		else OLED_ShowString(0,9,"STOP",OLED_6X8);
		
		if(Samplingmode)OLED_ShowString(0,18,"Co_",OLED_6X8);
		else OLED_ShowString(0,18,"Si_",OLED_6X8);
		
		OLED_ShowNum(0,27,WaP,2,OLED_6X8);	
		
		if(Trigger_Slope)OLED_ShowString(0,38,"-UP-",OLED_6X8);
		else OLED_ShowString(0,38,"DOWH",OLED_6X8);

		OLED_ShowFloatNum(-6,47,Threshold_voltage,2,1,OLED_6X8);//这里可能会提示警告，可以不管它	
		OLED_ShowFloatNum(-6,56,RangeDOWH,2,1,OLED_6X8);//这里可能会提示警告，可以不管它
		...
		if(key!=8)
		{
			...
			OLED_ShowString(52,55,"-",OLED_6X8);
		}
		...
		if(key!=8)
		{
			switch(Fre)
			{ 
				case 1:OLED_ShowString(103, 55, "50us",  OLED_6X8);break;
				case 2:OLED_ShowString(97,  55, "100us", OLED_6X8);break;
				case 3:OLED_ShowString(97,  55, "200us", OLED_6X8);break;
				case 4:OLED_ShowString(97,  55, "500us", OLED_6X8);break;
				case 5:OLED_ShowString(109, 55, "1ms",   OLED_6X8);break;
				case 6:OLED_ShowString(109, 55, "2ms",   OLED_6X8);break;
				case 7:OLED_ShowString(109, 55, "5ms",   OLED_6X8);break;
				case 8:OLED_ShowString(103, 55, "10ms",  OLED_6X8);break;
				case 9:OLED_ShowString(103, 55, "20ms",  OLED_6X8);break;
				case 10:OLED_ShowString(103, 55, "50ms",  OLED_6X8);break;
				case 11:OLED_ShowString(97,  55, "100ms", OLED_6X8);break;
				case 12:OLED_ShowString(97,  55, "200ms", OLED_6X8);break;
				case 13:OLED_ShowString(97,  55, "500ms", OLED_6X8);break;
		    }			
		}	
		 if(onkey){
			switch(key)
...
uint8_t DMA_accomplish=1;
uint16_t TIMPSC=72 - 1,TIMARR=30000 - 1,TIMCCR=1000;
...
{	
		...
		if(DMA_accomplish==1)
		{
			...
			{	
				for(uint8_t i=125;i<200;i++) { WaveForm[i] = 0;}
				DMA_Cmd(DMA1_Channel1, DISABLE);					//DMA失能，在写入传输计数器之前，需要DMA暂停工作
...
//		if(Fre<10)while(!DMA_accomplish);	//等待DMA工作完成
				switch(Fre)
				{ 				
					case 1: delay_ms(1+1);break; //5us
					case 2: delay_ms(2+1);break; //10us
					case 3: delay_ms(4+2);break; //20us
					case 4: delay_ms(10+2);break; //50us
					case 5: delay_ms(20+4);break; //0.1ms 100us
					case 6: delay_ms(40+8);break; //0.2ms 200us
					case 7: delay_ms(100);break; //0.5ms 500us
					case 8: delay_ms(100);break; //1ms
					case 9: delay_ms(100);break; //2ms
				}			
