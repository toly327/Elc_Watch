...
void AHT20_Get_data()
{
	...
	AHT20humidity = CT_data[0]*100*10/1024/1024;  //计算得到湿度值c1（放大了10倍,如果c1=523，表示现在湿度为52.3%）
	AHT20temperature = CT_data[1]*200*10/1024/1024-500;//计算得到温度值t1（放大了10倍，如果t1=245，表示现在温度为24.5℃）
...
void SDA_Pin_Output_High(void)   //将PA15配置为输出 ， 并设置为高电平， PA15作为I2C的SDA
...
void SDA_Pin_Output_Low(void)  //将PA15配置为输出  并设置为低电平
{	GPIO_InitTypeDef  GPIO_InitStruct;
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_OD;//开漏输出
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_15;
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
...
void SCL_Pin_Output_High(void) //SCL输出高电平，PB3作为I2C的SCL
